...
const login = async ()=>{
  const response = await fetch('https://dkmh.hcmuaf.edu.vn/api/auth/login', {
    ...
    body: 'username=20130282&password=saimatkhau1&grant_type=password'
});
...
app.get("/tkb", async (req,res)=>{
  const tokenUser = await login()
  let data = []
var headers = {
  'Accept': 'application/json, text/plain, */*',
  'Accept-Language': 'en-US,en;q=0.9,vi;q=0.8',
  'Authorization': 'Bearer '+ JSON.parse(tokenUser).access_token,
  'Connection': 'keep-alive',
